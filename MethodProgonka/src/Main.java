public class Main {
    ...
    public static double[][] progonkaMethod(double[][] matr) {
        ...
        for (int i = 0; i < length; i++) {
            if (i == 0) {
                ...
                if(i != length - 1) tempArr[i][0] = matr[i][i + 1] / (-matr[i][i] - matr[i][i - 1] * tempArr[i - 1][0]);
    ...
    public static double[][] matr(double a, double b, double c, int n, double x, double h, double A, double B){ // Метод для заполнения матрицы коэффициентами
    ...
    public static double[] arg(double[][] matr, double a1, double a2){ // Метод для нахождения значений функции в точках
        int n = matr.length;
