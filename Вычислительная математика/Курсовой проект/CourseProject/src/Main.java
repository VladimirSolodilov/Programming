public class Main {
    public static double[] Adams3(int N, double[] x) {
        double[] a3 = new double[N];
        a3[0] = 2;

        for (int i = 1; i < N; i++) {
            if (i <= 3) {
                a3[i] = a3[i - 1] + 0.01 * (Math.sin(x[i]) - a3[i - 1]);
            } else {
                a3[i] = a3[i - 1] + (0.01 / 12) * (23 * (Math.sin(x[i - 1]) - a3[i - 1])
    ...
    public static double[] Adams4(int N, double[] x) {
        double[] a4 = new double[N];
        a4[0] = 2;

        for (int i = 1; i < N; i++) {
            if (i <= 3) {
                a4[i] = a4[i - 1] + (0.01 * (Math.sin(x[i - 1] - a4[i - 1])));
            } else {
                a4[i] = a4[i - 1] + ((0.01 / 24) * (55 * (Math.sin(x[i - 1]) - a4[i - 1]) - 59 * (Math.sin(x[i - 2]) - a4[i - 2])
    ...
    public static double[] difFunctionValue(int N, double[] x) {
        double[] Adams3Value = Adams3(N, x);
    ...
    public static void print(int N) {
        ...
        for (int i = 0; i < N; i++) {
            if ((i > 0 && i < 10) || i % 10 == 0) {
                System.out.println("|" + i + "\t\t\t|" + String.format("%.3f", xValue(0.01, N)[i]) + "\t|" + String.format("%.5f", difFunctionValue(N, xValue(0.01, N))[i]) + "\t\t|"
                        + String.format("%.5f", Adams3(N, xValue(0.01, N))[i]) + "\t\t|" + String.format("%.5f", difFunctionValue(N, xValue(0.01, N))[i])
